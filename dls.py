...
def dls_recurs(graph, start_node, end_node, limit=1, visited=set(), level=0):
    order = []
    if level > limit:
        return []
        ...
        for node in graph[start_node]:
            if node not in visited:
                if (level + 1 == limit) and node not in order:
                    ...
                    new_order = dls_recurs(
                        graph, node, end_node, limit, visited, level + 1
                    )
...
def dls(graph, start_node, end_node, limit=1, visited=set(), level=0):
    order = dls_recurs(graph, start_node, end_node, limit, visited, level)
